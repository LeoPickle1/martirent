...
const maintenanceTypeNamesDe = {
  ...
  "Heat Pump Service": "Wärmepumpen-Service",
  "Boiler Heat Pump Check": "Boiler / Wärmepumpe prüfen",
...
const cancelContactForm = () => {
  ...
  const getSpreadsheetSummaryRows = () => {
    const rows = [
      ...
      ["�berf�llig", items.filter((item) => item.status === "overdue").length, "", "", ""],
      ["Nicht �berf�llig", items.filter((item) => item.status !== "overdue").length, "", "", ""],
      ["N�chste 30 Tage", items.filter((item) => item.days >= 0 && item.days <= 30).length, "", "", ""],
      ["N�chste 365 Tage", items.filter((item) => item.days >= 0 && item.days <= 365).length, "", "", ""],
    ];
